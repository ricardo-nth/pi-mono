...
export class WelcomeScreenComponent extends Container {
	constructor(config: WelcomeScreenConfig) {
		...
		// Render ASCII art with right content side by side, centered
		const gap = 3;
		const maxLines = Math.max(asciiLines.length, rightContent.length);

		// Calculate total width of the combined content
		const rightWidth = Math.max(...rightContent.map((line) => visibleWidth(line.replace(/\x1b\[[0-9;]*m/g, ""))));
		const combinedWidth = asciiWidth + gap + rightWidth;

		// Calculate left padding to center
		const leftPadding = Math.max(0, Math.floor((termWidth - combinedWidth) / 2));
		...
		for (let i = 0; i < maxLines; i++) {
			...
			const paddedAscii = asciiLine + " ".repeat(Math.max(0, asciiWidth - visibleWidth(asciiLine)));
			const combinedLine = padStr + theme.fg("accent", paddedAscii) + " ".repeat(gap) + rightLine;

			this.addChild(new Text(combinedLine, 0, 0));
		}
